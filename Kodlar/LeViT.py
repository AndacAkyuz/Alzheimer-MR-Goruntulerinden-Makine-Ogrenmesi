...
for fold, (train_index, val_index) in enumerate(kf.split(all_filepaths)):
    ...
    plt.title(f'Fold {fold + 1} - LeViT Confusion Matrix')
    ...
    plt.title(f'Fold {fold + 1} - LeViT Receiver Operating Characteristic')
    plt.legend(loc="lower right")
    plt.show()

    # Modeli kaydetme
    torch.save(model.state_dict(), f'levit_model_fold_{fold + 1}.pth')
